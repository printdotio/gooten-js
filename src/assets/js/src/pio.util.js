...
PIO.env = {};
PIO.env.isIE9 = window.navigator.userAgent.toUpperCase().indexOf("MSIE 9") !== -1;
PIO.env.isIE10 = !PIO.env.isIE9 && window.navigator.userAgent.toUpperCase().indexOf("MSIE 10") !== -1;
...
PIO.util.isUndefined = function(v) {
    return typeof v === "undefined";
};

PIO.util.or = function(v1, v2, v3) {
    var _isUndefined = PIO.util.isUndefined;
    if (!_isUndefined(v1))
        return v1;
    if (!_isUndefined(v2))
        return v2;
...
PIO.util.Mediator = function() {};


PIO.util.asPubSub = function() {
    var self = this;
    this._a = {};
    this._all = [];
...
PIO.util.Mediator = PIO.util.asPubSub;

PIO.util.randomizeArray = function(srcArray, outputLength) {
...
PIO.util.loadImage = function(url, cb) {
...
PIO.util.getImgSrvUrl = function(url) {
    if (!url) return url;
...
PIO.util.numericInputLess = function(data, prop) {
    return function() {
        ni(data, prop, -1);
    };
};

PIO.util.numericInputMore = function(data, prop) {
    return function() {
        ni(data, prop, 1);
    };
};

PIO.util.multiplyMatrices = function(m1, m2) {
    var result = [];
...
PIO.util.rotateVector = function(rotation, x, y) {
    var rot = rotation * -1;
    var multiply = PIO.util.multiplyMatrices;
    if (rot === 0)
        return {
            x: x,
            y: y
...
PIO.util.Scaler = function(sourceValue, destinationValue) {
    var dx = 1;
...
PIO.util.dom.getElRelationship = function(elA, elB) {
...
PIO.util.dom.getRotationFromMatrix = function(matrix) {
...
PIO.util.dom.getRotationAttrs = function() {
    return ["transform",
        "-webkit-transform",
        "-moz-transform",
        "-ms-transform",
...
PIO.util.dom.getRotation = function(el) {
    var d = PIO.util.dom;
...
PIO.util.dom.setRotation = function(el, val) {
    var a = PIO.util.dom.getRotationAttrs();
...
PIO.util.dom._parseTransform = function(str) {
    var b = '(';
...
PIO.util.dom.getTransforms = function(el) {
    var d = PIO.util.dom;
    var a = d.getRotationAttrs();
...
//
// thanks Photobucket
PIO.util.dom.getDataVal = function(el, propName) {
    if (el.dataset) {
...
PIO.util.dom.setDataVal = function(el, propName, value) {
    if (el.dataset) {
...
PIO.util.injectCss = function(url) {
    var d = document;
...
PIO.util.postMessage = function(data, origin) {
    var w = window;
...
PIO.util.getCdnUploadUrl = function(bucket, filename, maxWidth, maxHeight) {
    return "https://az795974.vo.msecnd.net/cdn/" + bucket + "/" + filename + "?mode=max&w=" + maxWidth + "&h=" + maxHeight;
};

PIO.util.copyArray = function(a){
  var f = [];
...
//    }, 0);
//}
//PIO.util.addPostMessageHandler = function (fn) {
//    var w = window;
//    if (PIO.env.isIE9) {
